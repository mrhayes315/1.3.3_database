...
with con:    
    ...
    input_var = raw_input("Enter the month you want to know what cities were warmest in: ")
    
    
    #Query to get city, state that are warmest in July
    cur.execute("SELECT name, state FROM cities INNER JOIN weather ON name = city WHERE warm_month = '{0}'".format(input_var))
    #Gets all the rows from that query so we can do something with them
    rows = cur.fetchall()
    
    #a string to start the sentence
    result = "The cities that are warmest in {0} are: ".format(input_var)
    
    #If there were no cities whose warmest month was the input, tell us
    if len(rows) == 0:
    	result = "There were no cities whose warmest month was in {0}.".format(input_var)
    else :
    	#for each of the rows in the result set, add the city and state to the sentence
    	for row in rows:
        	result = result + " " + row[0] + ", " + row[1] + ","
    
    #get rid of the trailing comma and add a period to make it a proper sentence (don't worry if there were no results, it removes and re-adds the period)
    result = result[:-1]
    result += '.'
    #print the sentence
    print result    
